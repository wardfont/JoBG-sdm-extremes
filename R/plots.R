...
spatial_plot_limited_ensemble <- function(katz_ensemble_gam_biovars_predictions,
                                          base_ensemble_gam_biovars_predictions,
                                          euforest, species, display_species, palette,
                                          terraclimate_sdm) {
  katz <- rast(lapply(
  ...
  one <- ggarrange(
    plotlist = plots_base, # ncol = 2, nrow = 2,
    common.legend = T
  )

  ggsave("../sdm-extremes-paper/plot/one_spatial/limit_final_one_zissou.svg", one,
    scale = 1, height = 297, width = 210, units = "mm"
  ) # A4
